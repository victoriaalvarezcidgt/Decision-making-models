...
\end{align*}
  ...
  Note that the objective function denoted with $\mathcal{L}(\phi)$ has two terms, with the first term -- $\displaystyle{\sum_i l(\hat{y}_i, y_i)}$ -- being the __loss function__ and the second term -- $\displaystyle{\sum_j \Omega (f_j)}$ -- being the __regularization__.
