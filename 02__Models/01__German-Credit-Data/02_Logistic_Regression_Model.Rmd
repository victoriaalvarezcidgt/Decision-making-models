...
View(credit_data)
## Dimension of the original dataset: row and column numbers
dim(credit_data)
```
## Preprocessing
...
credit_data <- credit_data %>%
  clean_names()
### Viewing the dataset after changing column names
head(credit_data)
```
...
The best accuracy will be seen near $c = 0.5$, and instead of manually checking cutoffs, we would create an ROC curve (receiver operating characteristic curve) that will sweep through all possible cutoffs, and plot the sensitivity and specificity.

```{r}
test_roc = roc(as.numeric(test_set$class) ~ as.numeric(predictions), plot = TRUE, print.auc = TRUE, 
              col = "maroon")
